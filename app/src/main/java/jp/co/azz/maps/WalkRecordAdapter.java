...
public class WalkRecordAdapter extends CursorAdapter {
    ...
    public void bindView(View view, Context context, Cursor cursor) {
        // Cursorからデータを取り出します
        String start_time = cursor.getString(cursor.getColumnIndexOrThrow(DatabaseContract.History.COLUMN_START_DATE));
        String end_time = cursor.getString(cursor.getColumnIndexOrThrow(DatabaseContract.History.COLUMN_END_DATE));
        double distance = cursor.getDouble(cursor.getColumnIndexOrThrow(DatabaseContract.History.COLUMN_DISTANCE));
        String step_cnt = cursor.getString(cursor.getColumnIndexOrThrow(DatabaseContract.History.COLUMN_NUMBER_OF_STEPS));
        String calorie = cursor.getString(cursor.getColumnIndexOrThrow(DatabaseContract.History.COLUMN_CALOLIE));
